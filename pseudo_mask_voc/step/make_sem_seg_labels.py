...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id // args.process_times):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            result_filename = os.path.join(args.sem_seg_out_dir, img_name + '.png')
            ...
            rw_pred = torch.argmax(rw_up_bg, dim=0).numpy()
            rw_pred = keys[rw_pred]

            UtilMy.gray_to_color(rw_pred.astype(np.uint8)).save(result_filename)
