...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id // args.process_times):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            result_name = os.path.join(args.ins_seg_out_dir, img_name + '.npy')
            ...
            instance_shape = torch.argmax(rw_up_bg, 0).numpy()
            ...
            detected = detect_instance(rw_up.numpy(), instance_shape, instance_class_id,
                                       max_fragment_size=size[0] * size[1] * 0.01)

            np.save(result_name, detected)
